 ...
 *	File name: my_lcd.c
 *
 *	Description: source file of the LCD
 *
 *	Author: FARID
 *
 ****************************************************************************************************************************************************/
...
#if ( LCD_FIRST_DATA_PIN_ID == 4 )
	/* Configure the 4 MSB in the data port as outputs */
...
#else
	lcd_port_value = ( lcd_port_value & 0xF0 ) | ( ( command & 0x0F ) >> 4 );
#endif
...
#else
	lcd_port_value = ( lcd_port_value & 0xF0 ) | ( ( data & 0x0F ) >> 4 );
#endif
	GPIO_writePort( LCD_DATA_PORT_ID, lcd_port_value );
	_delay_ms(1); /* delay for processing Tdsw = 100ns */
	GPIO_writePin( LCD_E_PORT_ID, LCD_E_PIN_ID, LOGIC_LOW ); /* Disable LCD E=0 */
